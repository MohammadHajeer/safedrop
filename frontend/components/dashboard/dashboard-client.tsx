...
function DashboardSkeleton() {
  return (
    <div className="space-y-8" aria-label="Loading dashboard">
      ...
      <div className="grid gap-4 lg:grid-cols-[0.72fr_0.72fr_1.56fr]">
...
export function DashboardClient() {
  ...
  const active = dashboard.active.data;
  const expired = dashboard.expired.data;
  const consumed = dashboard.consumed.data;
  const revoked = dashboard.revoked.data;
  const user = dashboard.user.data;
  const storage = dashboard.storage.data;

  if (!active || !expired || !consumed || !revoked || !user || !storage) {
  ...
  return (
    <div className="space-y-8" aria-busy={dashboard.isPending}>
      <header className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
        <div>
          ...
          <p className="mt-2 max-w-2xl leading-7 text-muted-foreground">
            Keep an eye on what is active, then share something new when you are
            ready.
          </p>
        </div>
        <ButtonLink
      ...
      <section
        aria-label="Drop statistics"
        className="grid gap-4 lg:grid-cols-[0.72fr_0.72fr_1.56fr]"
      >
        <Card className="gap-5 p-1 shadow-none ring-border">
          <CardContent className="p-5 sm:p-6">
            ...
            <p className="mt-7 text-3xl font-semibold tracking-tight tabular-nums">
              {active.total}
            </p>
            <p className="mt-1 text-sm font-medium">Active Drops</p>
            <p className="mt-2 text-sm leading-6 text-muted-foreground">
              Available to recipients right now.
            </p>
          </CardContent>
        </Card>
        <Card className="gap-5 p-1 shadow-none ring-border">
          ...
          <CardContent className="p-0">
            {recent.length ? (
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="mt-1 flex items-center gap-1.5 text-xs text-muted-foreground">
                          <Clock3Icon className="size-3.5" /> Created{" "}
                          {relativeDate(drop.created_at)}
                        </p>
                      </div>
                      <div className="hidden items-center gap-3 sm:flex">
                        <span className="inline-flex items-center gap-1 text-xs text-muted-foreground">
                          <EyeIcon className="size-3.5" /> {drop.view_count}/
                          {drop.max_views}
                        </span>
              ...
              <div className="px-6 py-10 text-center">
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  Your recent Drops will appear here.
                </p>
              </div>
            )}
          </CardContent>
        ...
        <Card className="relative overflow-hidden border-primary/20 bg-primary-soft/35 p-6 shadow-none ring-primary/10 sm:p-7">
          <div className="relative z-10">
            ...
            <p className="mt-3 leading-7 text-muted-foreground">
              Add a message, up to five files, and a view limit in one calm
              flow.
            </p>
            <ButtonLink
              href="/dashboard/drops/new"
